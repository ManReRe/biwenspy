...
def test_calibrate_starting_balance_backsolves_from_the_real_current_balance():
    # Owner has net +300 recorded (income 500 - expense 200). Their real current
    # balance (per the API, unaffected by any hidden-from-others setting) is
    # 38,470 -- so the season must have actually started at 38,470 - 300 = 38,170,
    # not analytics.STARTING_BALANCE (20,000,000). This is exactly the real-world
    # scenario this function exists for: a mid-season budget reset (e.g. a new
    # football season) that no board event reflects.
    conn = db.init_db(":memory:")
